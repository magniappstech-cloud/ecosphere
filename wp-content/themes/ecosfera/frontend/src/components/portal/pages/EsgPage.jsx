...
function filterEsg(items, topic, search, sort) {
...
export function EsgPage({ data }) {
  const esgPages = data?.collections?.esgPage || [];
  const topics = Array.from(new Set(esgPages.flatMap((article) => article.topics || [])));
  ...
  const filtered = filterEsg(esgPages, topic, search, sort);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(data?.rest?.esgSubmission, {
        method: 'POST',
    ...
    <>
      <header className="art-header">
        <div className="art-header__inner container">
          <div>
            <div className="sec-label">ESG</div>
            <h1 className="art-header__title">ESG</h1>
        ...
        <div className="art-filters container">
          <div className="art-search">
            <input
              type="search"
              placeholder="Search ESG..."
              aria-label="Search ESG"
              value={search}
      ...
      {view === 'list' ? (
        <div id="art-view-list" className="art-view">
          <div className="art-grid container" role="list" aria-label="ESG list">
            {filtered.length ? (
              filtered.map((article) => (
                <a key={article.id} className="art-card" href={article.url}>
                  ...
                  <div className="art-card__body">
                    <div className="art-kicker">{article.topics?.[0] || 'ESG'}</div>
                    <h3>{article.title}</h3>
              ...
              <div className="art-empty">No published ESG pages yet. Add the first record in WordPress admin.</div>
            )}
          </div>
